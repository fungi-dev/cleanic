...
namespace Cleanic.Application
{
    ...
    {
        public WriteAgent(ICommandBus cmdBus, IEventBus evtBus, IWriteRepository db, DomainMeta domain, Func<ServiceMeta, Service> serviceFactory)
        {
        ...
        private readonly Func<ServiceMeta, Service> _serviceFactory;
        ...
        private async Task HandleCommand(Command cmd)
        {
            ...
            await aggregate.Do(cmd, svcMetas.Select(x => _serviceFactory(x)));
        ...
        private async Task HandleEvent(Event @event)
        {
            ...
            {
                var svc = _serviceFactory(svcMeta);
