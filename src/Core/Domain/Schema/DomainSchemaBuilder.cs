namespace Cleanic.Core
{
    ...
    {
        public DomainSchemaBuilder(LanguageSchema languageSchema)
        {
            _languageSchema = languageSchema;
        }

        public DomainSchema BuildFromAssembly(Assembly assembly)
        {
        ...
        public DomainSchemaBuilder Add<T>()
        {
            var type = typeof(T).GetTypeInfo();

            if (type.IsSubclassOf(typeof(AggregateLogic)))
            {
        ...
        private IEnumerable<(AggregateInfo, Type)> FindAggregateLogicTypesInAssembly(Assembly domainAssembly)
        {
            var aggregateLogicTypes = domainAssembly.DefinedTypes.Where(x => x.IsSubclassOf(typeof(AggregateLogic<>)));
